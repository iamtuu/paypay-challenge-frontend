...
export default (apiUrl, httpClient = fetchJson) => {
    ...
    const convertHTTPResponseToREST = (response, type, resource, params) => {
        ...
        if (json && json.error && json.error.message.length > 0) {
        ...
        switch (type) {
            ...
            case GET_MANY_REFERENCE:
                if (!headers.has('content-range')) {
                    throw new Error('The content-range header is missing in the HTTP Response. The jsonServer REST client expects responses for lists of resources to contain this header with the total number of results to build the pagination. If you are using CORS, did you declare X-Total-Count in the Access-Control-Expose-Headers header?');
                }
                return {
                    data: json,
    ...
    return (type, resource, params) => {
        // simple-rest doesn't handle filters on UPDATE route,
        ...
        // simple-rest doesn't handle filters on DELETE route,
